...
export const orderStore = {
  async create(order: Omit<OrderEntity, "id" | "createdAt">): Promise<OrderEntity> {
  ...
  async listByUserId(userId: string): Promise<OrderEntity[]> {
    const orders = await OrderModel.find({ userId }).sort({ createdAt: -1 }).lean<OrderDb[]>().exec();
    return orders.map(toEntity);
  },

  async listAll(): Promise<OrderEntity[]> {
    const orders = await OrderModel.find().sort({ createdAt: -1 }).lean<OrderDb[]>().exec();
    return orders.map(toEntity);
  },

  async cancelByIdForUser(orderId: string, userId: string): Promise<OrderEntity | undefined> {
